...
using Moobie.WebView.Exceptions;
...
{
    public class IndexView : IViewBuilder
    {
        private readonly ConventionableName MyName = new ConventionableName("Index View".Split(' '));
        public ConventionableName Name => MyName;

        public PagePayload<T> BuildView<T>() where T : class
        {
            if (typeof(T) != typeof(IndexData))
            {
                throw new PayloadDataMismatchException($"Unable to build {MyName} with {typeof(T).Name} as the base data type.");
            }
            //TEMPORARY
            ...
            return result as PagePayload<T>;
        }
    }
}
